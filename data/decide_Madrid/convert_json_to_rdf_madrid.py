#!/usr/bin/env python3
"""
Script to convert Decide Madrid JSON data to RDF/OWL format
...
def convert_json_to_rdf(json_file, output_file):
    """Convert Decide Madrid JSON data to RDF/OWL format"""
    # Load JSON data
    ...
    for contribution in data["del:hasContribution"]:
        ...
        # Link to response if available
        if "del:responseTo" in contribution and "@id" in contribution["del:responseTo"]:
            response_id = contribution["del:responseTo"]["@id"]
            response_uri = URIRef(base_uri + response_id)
            g.add((contribution_uri, DEL.responseTo, response_uri))
...
def main():
    parser = argparse.ArgumentParser(description='Convert Decide Madrid JSON data to RDF/OWL format')
