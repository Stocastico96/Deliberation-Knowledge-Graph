#!/usr/bin/env python3
"""
Script to convert Habermas Machine JSON data to RDF/OWL format
...
def convert_json_to_rdf(json_file, output_file):
    """Convert Habermas Machine JSON data to RDF/OWL format"""
    # Load JSON data
    ...
    for contribution in data["del:hasContribution"]:
        ...
        # Link to argument if available
        if "del:hasArgument" in contribution and "@id" in contribution["del:hasArgument"]:
            argument_id = contribution["del:hasArgument"]["@id"]
            argument_uri = URIRef(base_uri + argument_id)
            g.add((contribution_uri, DEL.hasArgument, argument_uri))
        
        # Link contribution to process
        g.add((process_uri, DEL.hasContribution, contribution_uri))
    
    # Add arguments if available
    if "del:hasArgument" in data:
        for argument in data["del:hasArgument"]:
            argument_uri = URIRef(base_uri + argument["del:identifier"])
            g.add((argument_uri, RDF.type, DEL.Argument))
            g.add((argument_uri, DEL.identifier, Literal(argument["del:identifier"])))
            g.add((argument_uri, DEL.text, Literal(argument["del:text"])))
            
            # Link argument to process
            g.add((process_uri, DEL.hasArgument, argument_uri))
...
def main():
    parser = argparse.ArgumentParser(description='Convert Habermas Machine JSON data to RDF/OWL format')
